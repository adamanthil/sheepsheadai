"""Search corpus generation and the policy-iteration projection loop.

Corpus generation is tested with a scripted committee (deterministic
material targets, no real search); the projection's channel alignment,
loss masking and the retention anchor's zero-gradient-at-init property are
tested on real generated corpora with the same agent that produced them.
"""
...
def test_kd_kl_zero_at_anchor():
    ...
    student[:, 0] += 1.0
    assert float(kd_kl(anchor, student).min()) > 0.0
...
def test_channel_alignment_and_loss_masking():
    ...
    with torch.no_grad():
        ...
        anchored = set_flat == SET_CODES["retention"]
        ...
        # No explicit CE weight in a raw corpus: every row weighs 1.
        assert torch.all(weight_flat == 1.0)
...
def test_convention_telemetry_rates():
    ...
    batch = [(0, 2)]
    kinds = ["action"] * 3
    rows = policy_iteration.convention_rows(agent, batch, kinds)
    assert len(rows) == 3
    ...
    counts: dict = {}
    policy_iteration.accumulate_conventions(counts, rows, logits)
    report = policy_iteration.convention_report(counts)
...
def test_split_by_game_keeps_siblings_together():
    ...
    train, hold = policy_iteration.split_by_game(episodes, holdout_frac=0.25, seed=3)
    ...
    train2, hold2 = policy_iteration.split_by_game(episodes, holdout_frac=0.25, seed=3)
    assert [ep[0]["game"] for ep in hold2] == [ep[0]["game"] for ep in hold]
    with pytest.raises(SystemExit):
        policy_iteration.split_by_game(episodes[:7], 0.25, 3)
...
def test_run_epoch_steps_and_updates_weights():
    ...
    stats, steps = policy_iteration.run_epoch(agent, episodes, args, train=True)
    assert steps > 0
    ...
    policy_iteration.run_epoch(agent, episodes, args, train=False)
