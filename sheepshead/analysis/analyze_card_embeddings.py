...
def compute_embedding_delta(
    initial: torch.Tensor, trained: torch.Tensor
) -> torch.Tensor:
...
def write_embeddings_csv(
    path: Path, embeddings: torch.Tensor, card_names: list, card_ids: list
):
...
def analyze_card_movement(
    delta: torch.Tensor, card_ids: list, card_names: list
) -> Dict[str, float]:
...
def plot_dimension_importance(importance_dict: Dict[str, np.ndarray], filename: str):
    ...
    axes[0].bar(
        range(len(dim_labels)), importance_dict["mean_abs_change"], color="steelblue"
    )
    axes[0].set_ylabel("Mean |Δ|", fontsize=11)
    axes[0].set_title(
        "Mean Absolute Change per Dimension", fontsize=12, fontweight="bold"
    )
    ...
    axes[1].set_title(
        "Standard Deviation of Change per Dimension", fontsize=12, fontweight="bold"
    )
    ...
    axes[2].bar(
        ...
        importance_dict["max_abs_change"],
        color="mediumseagreen",
    )
    axes[2].set_ylabel("Max |Δ|", fontsize=11)
    axes[2].set_title(
        "Maximum Absolute Change per Dimension", fontsize=12, fontweight="bold"
    )
...
def plot_card_movement(movements: Dict[str, float], filename: str):
    ...
    ax.set_title(
        "Embedding Movement per Card (Sorted by Magnitude)",
        fontsize=13,
        fontweight="bold",
    )
...
def plot_trump_vs_fail_analysis(
    ...
    fail_ids = [
        ...
        if (name not in TRUMP and name not in ("PAD", "UNDER"))
    ]
    special_ids = [
        card_ids[i] for i, name in enumerate(card_names) if name in ("PAD", "UNDER")
    ]
    ...
    if special_ids:
        ...
        axes[2, 1].bar(
            range(len(special_trained)), special_trained, color="black", alpha=0.7
        )
...
def plot_special_cards(
    ...
    fig, axes = plt.subplots(
        len(special_groups), 1, figsize=(12, 4 * len(special_groups))
    )

    for idx, (group_name, group_cards) in enumerate(special_groups.items()):
        group_ids = [
            card_ids[card_names.index(card)]
        ...
        im = ax.imshow(
            ...
            aspect="auto",
            cmap="RdBu_r",
            vmin=-abs(group_delta).max(),
            vmax=abs(group_delta).max(),
        )
...
def print_summary_statistics(
    initial: torch.Tensor, trained: torch.Tensor, delta: torch.Tensor, card_ids: list
):
    ...
    print("\nDimension Analysis:")
    print(
        f"  Most changed dimension:   D{most_changed_dim} (mean |Δ| = {dim_mean_abs[most_changed_dim]:.6f})"
    )
    print(
        f"  Least changed dimension:  D{least_changed_dim} (mean |Δ| = {dim_mean_abs[least_changed_dim]:.6f})"
    )
...
def main():
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="sheepshead/analysis/card_embedding_analysis",
    ...
    print_summary_statistics(
        initial_embeddings, trained_embeddings, delta_embeddings, card_ids
    )
    ...
    write_embeddings_csv(
        output_dir / "initial_embeddings_all.csv",
        initial_embeddings,
        card_names,
        card_ids,
    )
    write_embeddings_csv(
        output_dir / "final_embeddings_all.csv",
        trained_embeddings,
        card_names,
        card_ids,
    )
    ...
    write_embeddings_csv(
        output_dir / "initial_embeddings_real_cards.csv",
        initial_embeddings,
        real_names,
        real_ids,
    )
