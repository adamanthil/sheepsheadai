...
class ForwardOutputs(NamedTuple):
    """Return type of PPOAgent._forward_vectorized: per-segment (B, T, ...)
    actor/critic/aux-head outputs for one minibatch."""

    logits_bt: torch.Tensor
    values_bt: torch.Tensor
    win_logits_bt: torch.Tensor
    returns_pred_bt: torch.Tensor
    secret_logits_bt: torch.Tensor
    points_pred_bt: torch.Tensor
    seen_trump_mask_logits_bt: torch.Tensor
    unseen_trump_higher_than_hand_logits_bt: torch.Tensor
...
class PPOAgent:
    ...
    def store_episode_events(self, events: list) -> None:
        ...
        for idx, ev in enumerate(events):
            if ev.get("kind") == "action":
                ...
                # CE teacher target (CE_Teacher_Design §2): the emission
                # ships a distribution over sorted(valid_actions); densify
                # to action_size here (zeros off the legal set) so the
                # update path consumes one fixed-width tensor. A missing or
                # length-mismatched target hardens the row to unlabeled —
                # an all-zero target would otherwise contribute a silent
                # zero CE loss while still counting as labeled (the
                # attempt-5a disarm mode, Search_Teacher_Design §10.3).
                raw_target = ev.get("search_target")
                sorted_valid = sorted(ev["valid_actions"])
                has_search_target = (
                    bool(ev.get("has_search_target", False))
    ...
    def _build_minibatch_tensors(self, batch, states, masks_t, kinds):
        ...
        for seg_start, seg_end in batch:
            ...
            for i in ev_range:
                ...
                target_lbl = (ev.get("search_target") if is_act else None) or [
                    0.0
                ] * self.action_size
                search_target_bt.append(
                    torch.tensor(target_lbl, dtype=torch.float32, device=device)
                )
        ...
        # Padded rows carry an all-zero target and has_search 0.0: the CE
        # loss selects rows by has_search, so padding contributes nothing.
    ...
    @staticmethod
    def _flatten_action_steps(minibatch: MinibatchTensors, forward: ForwardOutputs):
        flat_mask = minibatch.is_action_bt.view(-1)
        if flat_mask.sum() == 0:
            return None
        ...
        return FlattenedActionSteps(
            logits_flat=action_rows(forward.logits_bt),
            values_flat=action_rows(forward.values_bt),
            actions_flat=action_rows(minibatch.actions_bt),
            old_log_probs_flat=action_rows(minibatch.old_log_probs_bt),
            old_value_flat=action_rows(minibatch.old_value_bt),
            returns_flat=action_rows(minibatch.returns_bt),
            advantages_flat=action_rows(minibatch.advantages_bt),
            win_logits_flat=action_rows(forward.win_logits_bt),
            returns_pred_flat=action_rows(forward.returns_pred_bt),
            win_labels_flat=action_rows(minibatch.win_bt),
    ...
    def load_network_states(self, checkpoint, source: str = "<checkpoint>"):
        ...
        (league_worker.league_worker_play) go through here, so the non-strict
