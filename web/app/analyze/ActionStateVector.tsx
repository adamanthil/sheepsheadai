...
export default function ActionStateVector({ action }: ActionStateVectorProps) {
  ...
  const rows: Row[] = useMemo(() => {
    ...
    const pushPrivate = (
      ...
      for (let i = 0; i < count; i++) {
        ...
        const suit = cardLabel.endsWith("C")
          ? "C"
          : cardLabel.endsWith("S")
            ? "S"
            : cardLabel.endsWith("H")
              ? "H"
              : "D";
        ...
        r.push({
          idx: gi,
          value: state[gi] ?? 0,
          micro: `${labelPrefix} — ${withGlyph}`,
          sub,
          cat: "Private Card Indicators",
        });
      }
    };
    ...
    for (let rel = 0; rel < 5; rel++) {
      ...
      for (let j = 0; j < 34; j++) {
        ...
        r.push({
          ...
          micro: cardDesc,
          sub: seatLabels[rel],
          cat: "Current Trick Block",
        });
      }
      r.push({
        idx: seatStart + 34,
        value: state[seatStart + 34] ?? 0,
        micro: "Role flag — is_picker (0/1)",
        sub: seatLabels[rel],
        cat: "Current Trick Block",
      });
      r.push({
        idx: seatStart + 35,
        value: state[seatStart + 35] ?? 0,
        micro: "Role flag — is_known_partner (0/1)",
        sub: seatLabels[rel],
        cat: "Current Trick Block",
      });
    }
  ...
  const {
    ...
    return {
      subRowSpan: subRowSpanMap,
      catRowSpan: catRowSpanMap,
      rowClasses: classes,
      subStartIdx: subStart,
      catStartIdx: catStart,
      subZeroOnly: subZero,
    };
  }, [rows]);
              ...
              <table className={styles.stateVectorTable}>
                ...
                <tbody>
                  {rows.map((row, i) => {
                    ...
                    const groupClass =
                      `${rowClasses[i]} ${subStartIdx[i] ? styles.subGroupStart : ""} ${catStartIdx[i] ? styles.catGroupStart : ""}`.trim();
                    let subAnchor = i;
                    while (
                      subAnchor > 0 &&
                      rows[subAnchor - 1].sub === row.sub &&
                      rows[subAnchor - 1].cat === row.cat
                    )
                      subAnchor--;
                    const dimClass = subZeroOnly[subAnchor]
                      ? styles.subGroupDim
                      : "";
                    return (
                      <tr
                        key={row.idx}
                        className={`${groupClass} ${dimClass}`.trim()}
                      >
                        <td
                          className={`${styles.stateVectorIndex} ${row.value ? styles.cellHighlighted : ""}`}
                        >
                          {row.idx}
                        </td>
                        <td
                          className={`${styles.stateVectorValue} ${row.value ? styles.valueHighlighted : ""}`}
                        >
                          {row.value}
                        </td>
                        <td
                          className={`${styles.stateVectorMicroCat} ${row.value ? styles.cellHighlighted : ""}`}
                        >
                          {row.micro}
                        </td>
                        {showSub && (
                          <td
                            className={styles.stateVectorSubCat}
                            rowSpan={subRowSpan[i]}
                          >
                            {row.sub}
                          </td>
                        )}
                        {showCat && (
                          <td
                            className={styles.stateVectorCategory}
