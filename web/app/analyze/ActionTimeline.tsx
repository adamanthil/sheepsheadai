...
export default function ActionTimeline({
  ...
  const displayedStepRewards = trace.map((action) => {
    ...
    if (typeof base === "number" && typeof head === "number")
      return base + shapingWeight * head;
  ...
  const displayedTrace: AnalyzeActionDetail[] = trace.map((action, i) => ({
    ...
    discountedReturn:
      typeof discountedByIndex[i] === "number"
        ? discountedByIndex[i]
  ...
  const stepRewardValues = displayedTrace
    .map((action) =>
      typeof action.stepReward === "number" ? action.stepReward : null,
    )
    .filter((v): v is number => v !== null);
  const minStepReward =
    stepRewardValues.length > 0 ? Math.min(...stepRewardValues) : 0;
  const maxStepReward =
    stepRewardValues.length > 0 ? Math.max(...stepRewardValues) : 0;
