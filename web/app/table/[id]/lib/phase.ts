...
export function derivePhase(msg: TableStateMsg): { phase: TablePhase; isLeaster: boolean } {
 ...
 */
export function getSeatRole(msg: TableStateMsg, absSeat: number, started: boolean): SeatRole {
  const picker = msg.view.picker || 0;
...
export function interludeMode(validActionStrings: Set<string>): InterludeMode {
  ...
  for (const label of validActionStrings) {
    if (label.startsWith('CALL') || label === 'ALONE' || label.startsWith('JD')) return 'call';
  }
  return 'waiting';
}
