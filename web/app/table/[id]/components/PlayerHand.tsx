...
export default function PlayerHand({
  hand, isYourTurn, phase, yourMode, validActionStrings, onCardClick, isMobile,
}: PlayerHandProps) {
  ...
  return (
    <div className={`${styles.wrap} ${isMobile ? styles.mobWrap : styles.deskWrap}`}>
      <div className={styles.metaRow}>
        <div style={{ minWidth: 0 }}>
          <div className={ds.overline} style={{ fontSize: isMobile ? 9 : 11 }}>Your hand · {hand.length} cards</div>
          <div className={styles.metaText} style={{ fontSize: isMobile ? 12 : 16 }}>{meta}</div>
        </div>
        {isYourTurn && phase === 'play' && (
          <span className={`${ds.badge} ${ds.badgeAccent2}`} style={{ fontSize: 10 }}>● Your turn</span>
        )}
        {buryChosen && (
          <span className={`${ds.badge} ${ds.badgeAccent}`} style={{ fontSize: 10 }}>tap to bury</span>
        )}
      </div>
