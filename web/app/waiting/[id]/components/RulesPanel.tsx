...
function Segmented({ options, value, onChange, disabled }: SegmentedProps) {
  return (
    <div className={styles.segmented} style={{ gridTemplateColumns: `repeat(${options.length}, 1fr)` }}>
      {options.map((o, i) => (
        <button
          key={o}
          type="button"
...
export default function RulesPanel({
  partnerMode, scoringMode, onPartnerMode, onScoringMode, variant = 'panel',
}: RulesPanelProps) {
  ...
  return (
    <div className={inline ? styles.inline : `${ds.panel} ${styles.panel}`}>
      {!inline && (
        <div>
          <div className={ds.overline} style={{ marginBottom: 4 }}>House Rules · Host decides</div>
          <div className={styles.title}>Game Mode</div>
        </div>
      )}

      <div className={styles.group}>
        <div className={styles.groupLabel}>{inline ? 'Partner' : 'Partner selection'}</div>
        <Segmented
