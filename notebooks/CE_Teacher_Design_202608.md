...
§20.13 ADDENDUM 30 — AVENUE 2 RESULTS: aggregation adds nothing to the
optimised recipe; the two-generation window reads slightly below corpus
D alone (2026-09-12, 19:00; pipeline_agg.py; both arms optimised recipe,
3 trunk epochs @3e-5, head 4-7, lambda_ret 10; 8000 deals vs iter11,
seed-42 deals; paired reference D8k_t6 = corpus D alone, +0.0043 C_play):

    arm   corpus                          rows    full read           D_bid     C_play (PRIMARY)     paired C_play − D8k_t6   called-suit
    A2a   D ∪ twin (same gen, 256+1024)  175k   +0.0042 ± 0.0029   +0.0007   +0.0035 ± 0.0028      −0.0009 ± 0.0027        46.5
    A2b   D ∪ q re-anchored (2 gens)     190k   +0.0000 ± 0.0028   +0.0000   −0.0009 ± 0.0027      −0.0052 ± 0.0027 (−1.9σ) 48.6
    (A2a: called −0.0006 / jd +0.0075; A2b: called −0.0053 / jd +0.0036;
     A2b paired full vs D8k_t6 −0.0032 ± 0.0032, vs D8k_t3 −0.0034.)

Reads.
  1. A2a (pre-reg pass ≥ +0.004): FAILS at −0.0009. Same-generation
     volume beyond 140k rows, even with a 1024-iteration slice, does not
     move the optimised recipe; the twin's 1024 rows did not restore the
     convention either (46.5). Rows are saturated at this recipe; the
     lever that moved EV was trunk optimisation (add. 25-27).
  2. A2b (pre-reg pass ≥ +0.004; harm ≤ −0.004): reads −0.0052 ± 0.0027
     on the play route paired vs D8k_t6 — at the harm line (1.9σ), with
     the loss in CALLED mode. The AlphaZero window, in our prior-relative
     target form with the prior refreshed to theta_1, does not help and
     probably hurts. Instrument checks: bidding clean (D_bid 0.0000);
     noise-variance scale of q rows comparable to D's (p50 2.0e-4 vs
     2.5e-4; z p50 1.15 vs 1.44) — not a weight-scale artifact. The
     remaining explanation is staleness: q's Q was evaluated with
     theta_0 playing the continuations and on theta_0-era trajectories;
     at states iteration 1 already changed, that evidence points the
     wrong way for theta_1 (the E8 mechanism in reverse: the lead's
     value depends on the continuation, and the continuation moved).
  3. Consequence for the RC recipe: ONE generation of corpus per
     iteration, generated by the current theta_k with committee acting;
     no replay window. AlphaZero's window works with absolute visit
     targets and a value head that re-evaluates old states; ours are
     continuation-dependent advantages, which do not transfer across
