...
| 09-21 | `202609_recall_rc` GEN 2 (first generation at ×2.5; 54.6 h at 5.1 eps/s): h2h vs gen 1 +0.0154 ± 0.0108 (confirm seed +0.0147 ± 0.0108) — NOT significant, as expected after gen 1's one-time pick dividend (09-19 rows); PANEL-A +0.2152 [+0.186, +0.244] from +0.178 (review gate ≥ +0.06 cleared by a wide margin); PANEL-B −0.0438 [−0.069, −0.018] from −0.096 — both panels moved more than the h2h against gen 1 itself (+0.037 / +0.053 vs +0.015), i.e. the gen-2 gain is against the stronger external agents rather than against its own predecessor. Conventions: partner trump lead 94.5, t0 defender trump lead 0.7, called-suit lead 64.1 → 75.7 (sharpened), pick 36.6, alone 18.7, leaster 5.5. Aux (1,332 games): seen-trump acc 90.56 → 93.64, false-seen 14.34 → 10.76, recall t1-5 85.0/91.4/95.8/98.3/~100 (from 77.9/87.2/94.3/98.0), false-seen t1-5 8.5/15.3/21.0/25.3/26.8 (from 9.4/19.5/31.0/39.6/40.6) — the late-trick false-seen (the picker-bury/blind confusion) fell by a third; secret 99.99, points MAE 0.936, unseen-higher 99.73 — NOT READY on seen-trump alone. Coefficient pace: raw seen-trump loss 0.193 → 0.126 per update over the generation (−35 %); trainer greedy probes 1.0M → 2.0M acc +3.0 / false-seen −3.4 per million vs v2's 0.7M → 1.7M +0.7 / −0.8 and v2's best million (1.7M → 2.7M) +1.9 / −4.6; at 2.0M ours sits exactly on v2's 1.7M point (93.7 / 10.8) after being > 1M behind at gen 1 — ×2.5 closed most of the lag but did not beat v2's fastest stretch. Decision: `continue (below min_generations floor (3))`; gen 3 launched 16:49 with ×2.5 (decision ~00:50 09-24). Expectation for the phase: the aux gate, not the improving test, holds the league open — seen-trump bar most likely gen 4-5 on v2's curve | 4.3, 5.4, 7.1 |
| 09-21 | PANEL-A ladder on identical deals (operator asked how gen 2 compares to earlier artifacts). Same seed 42 / deal hash in both orchestrators; the reference agents scored with `eval_endpoint` under the orchestrator's protocol (single checkpoint repeated ×3, 3,996 deals; `runs/202609_recall_rc/recall_compare/panelA_{30m,release,iter11p1}.npz`). Every game seats all four anchors (drawn without replacement), so the 30M faces the same table as every other candidate — its membership does not depress its own score. Ladder (bootstrap SE ≈ 0.015): iter11 P1 +0.266 (called +0.233 / jd +0.299); v2 release +0.262 (+0.221 / +0.304); v2 8M = retention gen 8 +0.241 (+0.203 / +0.279); v2 retention gen 5 +0.222; **ours gen 2 +0.215** (+0.176 / +0.255); production 30M +0.196 (+0.183 / +0.210); ours gen 1 +0.178; v2 retention gen 2 +0.132; v2 gen 1 +0.043; seeds ours −0.065 / v2 −0.077. Reading: at 2M league episodes ours is level with the 30M on PANEL-A (+0.019, ~1 SE) and one SE below the v2 8M, with the two strongest artifacts (release, iter11 P1) ~0.05 above — consistent with PANEL-B −0.044 (the mean against exactly those four). The panel compresses at this level (its anchors are weak relative to the candidates; §5.3), so PANEL-B and direct h2h are the yardsticks from here. REFERENCE TABLE: `notebooks/panel_reference.csv` — every PANEL-A / PANEL-B read on the identical-deal protocol (this run's seed and generations, the retention run's gens 0-8, the 30M, the v2 release, iter11 P1, the v2 8M and both seeds) with mean / CI / SE / per-mode means / checkpoints / source npz; regenerated from the endpoint npz files by `uv run python -m sheepshead.analysis.build_panel_reference` (register a new read's npz there, never retype a number) | 4.3, 5.3 |
| 09-21 | PANEL-B ladder (operator: does PANEL-B compress at the bottom the way PANEL-A does at the top, and would the average of the two be a better wide-range yardstick?). Six new PANEL-B reads under the orchestrator's protocol (`recall_compare/panelB_*.npz`; all rows in `notebooks/panel_reference.csv`), beside PANEL-A on identical deals (SE ≈ 0.011-0.020): ours seed A −0.065 / B −0.419; v2 warm start −0.077 / −0.386; ours gen 1 +0.178 / −0.096; 30M +0.196 / −0.041; ours gen 2 +0.215 / −0.044; v2 8M +0.241 / −0.018; v2 release +0.262 / +0.014; iter11 P1 +0.266 / +0.006. Findings: (1) NO floor compression on PANEL-B in the range we occupy — the seeds sit at −0.4 with the floor far below, and the seed → gen-1 step is LARGER on B (+0.32) than on A (+0.24); (2) at the top the two panels separate the strong agents about equally (30M → iter11 P1: A +0.070, B +0.047; gen 2 → v2 8M: +0.026 on both), so the §5.3 premise that PANEL-A compresses at strong skill is NOT visible on these agents — PANEL-B's value is the different ecology (its members' B scores are a near-zero-sum round robin: release +0.014, iter11 +0.006, 8M −0.018, 30M −0.041), not extra range; (3) the two panels rank the eight agents identically except two within-noise swaps (release/iter11, 30M/gen 2), and ours gen 2 is level with the 30M on BOTH panels. Averaging: buys √2 precision at most and hides ecology divergences like gen 1's (A +0.18 / B −0.10); RECORD both, cite the average only as a summary, keep the pre-registered bars on PANEL-A | 5.3 |
| 09-24 | `202609_recall_rc` GEN 3 (machine crash 09-23 ~19:15 at episode 2.893M; orchestrator restarted 21:54, resumed from checkpoint_2850000 — 43k episodes and 2.7 h lost, nothing else; the trainer trimmed the orphaned progress rows and resumed the entropy-controller sidecar): h2h vs gen 2 +0.0277 ± 0.0117 — clears the marginal-value bar (edge ≥ 0.02, 2-SE lower bound +0.004 > 0) so the FIRST live application of the rule past the min_generations floor is `continue`, NO play-target step; PANEL-A +0.2104 [+0.180, +0.240] (from +0.215, flat within 1 SE), PANEL-B −0.0570 [−0.084, −0.030] (from −0.044, −0.013, 1 SE); conventions partner trump lead 98.1, t0 defender trump lead 1.9, called-suit 73.9, pick 36.3, alone 17.5, leaster 6.1. Aux: seen-trump acc 93.64 → 95.40, false-seen 10.76 → 8.29, recall t1-5 89.7/93.5/96.8/98.7/~100, false-seen t1-5 5.8/11.8/17.7/21.5/22.5 (from 8.5/15.3/21.0/25.3/26.8); secret 99.98, points MAE 0.953, unseen-higher 99.91 — NOT READY on seen-trump alone. Pace: raw seen-trump loss 0.126 → 0.084 per update (−33 %, the same proportional fall as gen 2); per million +1.8 acc / −2.5 false-seen (gen 2: +3.0 / −3.4) — the trainer's 50k probes show the improvement front-loaded (false-seen 10.8 → 6.5 by 2.75M, then 8.7 at 3.0M: 200-game probe noise ≈ ±1.5). Versus v2's curve: at 3.0M ours (95.4 / 8.3) sits between v2's 1.7M (93.8 / 10.8) and 2.7M (95.7 / 6.2) points — ≈ 2.5M-equivalent, i.e. still ~0.5M behind v2 in absolute episodes, lag no longer shrinking. Bar (≤ 0.5 false-seen) on v2's slope from here ≈ 2M+ episodes more → gen 5-6. Gen 4 launched 08:17 (×2.5) at 4.6 eps/s → boundary ≈ 20:50 09-26. Skill reading: the h2h keeps clearing the bar by small margins while both panels are flat; the gate, not skill, decides the phase length | 4.3, 5.4, 7.1 |
