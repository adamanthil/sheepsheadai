...
## E6 — Where is C2 NOT optimal? (exception atlas; pre-registered 2026-07-30)

**Question (operator-posed):** the human shortcut is "always lead the called
suit"; the E2 rung-2 result and the 2026-07-30 top-10 audit
(Learning_System_Redesign §7.12) both say that is right *in the large
majority* of cases but not pointwise. Which node features predict the
exceptions, and what does search do instead — i.e., what is the actual shape
of the optimal deviation class?

**Population:** all 321 primary C2-eligible nodes (141 adherent + 180
violating; under-call excluded) from the 500-seed deterministic self-play
scan of the retention 2.8M checkpoint
(`runs/called_suit_nodes_2800k.json`). Unconditional on the policy's choice,
so exception rates are P(conv suboptimal | features), not violation-biased.
Known selection caveats: node population is generated by the 2.8M policy;
search field/continuations are the same policy (labels are policy-relative,
rung-1 hindsight grade in that specific sense); one checkpoint.

**Labeling run (launched 2026-07-30, before this section's hypotheses were
tested against any label):** `counterfactual_called_suit_leads --cases` at
4096 iters-to-terminal, frac=1.0, 200 rollouts, 512 belief worlds — the
offline-grade budget; pi_gumbel argmax is the PRIMARY verdict (adopted
readout, Search_Readout_Comparison). Out:
`runs/convention_optimality_202607/cf_called_suit_atlas_2800k.json` (~28h
niced, measured 5.2 min/case).

**Label rule:** EXCEPTION = ESS-ok search whose pi_gumbel argmax is a
non-called-suit lead. Confidence tiers: tier A when the belief-MC Δ sign
agrees with the verdict (or |Δ| < 1 per-case SE → neutral counts with the
verdict); tier B otherwise. Non-reproducing/skipped nodes reported, never
imputed. AGREE-group nodes double as the sanity cell (E2's rung-2 found
conv-where-followed strongly positive; a high exception rate *among adherent
nodes* at 2σ would indict the measurement, not the convention).

**Pre-registered hypotheses** (H2/H3/H4 were sighted in the 10-case pilot —
disclosed as such; the pilot is excluded from confirmation counts only if the
atlas is marginal, since it is a subset of the same population):

- **H1 trump-void:** handTrumpCount == 0 ⇒ exception rate ↑ (can neither
  ruff the surfaced ace later nor exploit the partner reveal; pilot late-trick
  motif: 66% of late violators trump-void).
- **H2 forced donation:** minimum card points across the legal called-suit
  leads ≥ 10 (only fat called cards held) ⇒ exception ↑ — leading donates
  10+ points into the partner's ace.
- **H3 side-ace timing:** hand holds an off-suit fail ace ⇒ exception ↑
  (cash the sure trick first; deviation is a re-ordering, not a refusal).
- **H4 position:** exceptions concentrate at picker+4 and picker+1 vs
  +2/+3 (pilot per-position MC Δ: −0.26/+0.47/+0.49/−0.36).
- **H5 called-suit length:** ≥3 called-suit cards held ⇒ exception ↑
  (holder knows the suit is long/safe; partner-ID value lower, donation
  risk repeated).
- **H6 late-trick:** trickIndex ≥ 2 with suit still unled ⇒ exception ↑
  (partner-ID tempo value decayed; interacts with H1 — H1×H6 is the one
  pre-registered interaction).

**Readout:** per-feature exception-rate splits with Wilson 95% CIs
(`called_suit_exception_report.py`); replacement-class buckets of the
search-preferred card (low fail same suit as the policy's fat alternative /
off-suit low fail / off-suit fat fail / trump); pooled exception rate with
