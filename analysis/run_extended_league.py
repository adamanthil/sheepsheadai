...
class Orchestrator:
    ...
    def _write_generations_csv(self) -> None:
        ...
        with open(path, "w", newline="") as f:
            ...
            w.writeheader()
            streak = 0
            for g in sorted(int(k) for k in self.state["generations"]):
                rec = self.state["generations"][str(g)]
                ...
                row = {
                    ...
                    "ckpt": os.path.basename(self.boundary_ckpt(g)),
                    ...
                    "flat": v["flat"],
                    "flat_streak": streak,
                    "anchor_coeff": self.state["anchor_coeff"] if g == 1 else 0.0,
    ...
    def _write_report_md(self) -> None:
        ...
        lines = [
            ...
            "## Configuration",
            "",
            f"- arch `{self.args.arch}`, critic `{self.args.critic_mode}`, "
            f"resume `{self.args.resume}` (episode {s['resume_episode']:,})",
            f"- generation length {self.args.main_episodes:,} eps; "
        ...
        if s["events"]:
            ...
            lines.append("")
        with open(os.path.join(self.orch_dir, "report.md"), "w") as f:
            f.write("\n".join(lines))
    ...
    def _write_curve_png(self) -> None:
        ...
        conf = self.state.get("confirmation")
        ...
        fig.tight_layout()
        fig.savefig(os.path.join(self.orch_dir, "generations_curve.png"), dpi=150)
        plt.close(fig)
