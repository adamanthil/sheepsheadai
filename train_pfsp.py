...
def train_pfsp(num_episodes: int = 500000,
    ...
    for episode in range(start_episode + 1, num_episodes + 1):
        ...
        if transitions_since_update >= update_interval:
            ...
            # Entropy decay
            entropy_play_start, entropy_play_end = 0.02, 0.008
            entropy_pick_start, entropy_pick_end = 0.02, 0.010
            entropy_bury_start, entropy_bury_end = 0.02, 0.006
        ...
        if episode % cross_eval_interval == 0:
            ...
            for mode in [PARTNER_BY_JD, PARTNER_BY_CALLED_ACE]:
                eval_stats = population.run_cross_evaluation(mode, n_games_per_pair=20, max_agents=15)
                if eval_stats:
